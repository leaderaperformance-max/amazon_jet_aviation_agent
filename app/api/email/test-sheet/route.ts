...
export async function GET(req: NextRequest) {
  ...
  try {
    const token = await getAccessToken(acc as EmailAccountRow)
  ...
  try {
    sheetResult = await createPartsSheet({
      customerName: 'Test ' + new Date().toISOString().slice(11, 19),
      customerPhone: '+5511999999999',
      items: [{ part_number: 'MS21266-2N', quantity: '4' }],
      urgency: 'rotina',
    })
  } catch (err) {
    sheetResult = { error: (err as Error).message.slice(0, 800) }
  }
